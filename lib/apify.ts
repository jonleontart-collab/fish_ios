...
export async function enrichPlacesInBackground(
  ...
  try {
    ...
    // Poll for completion (Google Places crawler is usually fast for small batches)
    ...
    for (let i = 0; i < places.length; i++) {
      ...
      const queryStr = queries[i].toLowerCase();
      
      let item = items.find((it) => {
        const itemSearchStr = (it.searchString as string || "").toLowerCase();
      ...
      if (item) {
        const imageUrl = (item.imageUrls as string[])?.[0] ?? (item.image as string) ?? null;
        if (imageUrl) {
